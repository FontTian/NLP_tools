...
class FontCN_NLPtools():
    ...
    def clearText(self, NewStopWordsPath=False):

        if NewStopWordsPath != False:
    ...
    def jiebaCutStrpos(self,Delimiter = '/',NewText = 'False'):
        from jieba import posseg
        # 分词+词性
        if NewText == False:
            cutstr = posseg.cut(self.__text)
        else:
            cutstr = posseg.cut(NewText)
        result = ""
        for word, flag in cutstr:
            result += word + Delimiter + flag + ' '
        return result